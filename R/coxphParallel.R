  ...
  cluster)
{
  ...
  foreach(l = 1:blocks, .combine = rbind, .multicombine = TRUE, .inorder = FALSE, .packages = c("data.table", "doParallel", "parallel", "doMC", "foreach", "BiocParallel")) %dopar% {
    ...
    if (l==1) {
      ...
      if (system == "Windows") {
        wObjects <- parLapply(cl, wObjects, function(x) data.frame(rownames(x), x))
      } else {
      ...
      if (system == "Windows") {
        p <- parLapply(cl, models, function(x) data.frame(x$logtest, x$waldtest, x$sctest)["pvalue",])
      } else {
      ...
      # set colnames
      colnames(wObjects) <- c("Variable", "Beta", "ExpBeta", "StandardError", "Z", "P", "LRT", "Wald", "LogRank")
